    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out _hit, 1000f, layer))
                _cubeSelection = Instantiate(cube, new Vector3(_hit.point.x, 9, _hit.point.z), Quaternion.identity);
        ...
        if (_cubeSelection)
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit hitDrag, 1000f, layer))
            {
                ...
                _cubeSelection.transform.localScale = new Vector3(Mathf.Abs(xScale), 9, Mathf.Abs(zScale));
            }
        }
